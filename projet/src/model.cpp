...
#pragma omp critical
    ...
    if (coord.column < m_geometry - 1) {
      double tirage =
          pseudo_random(key * 13427 * 13427 + m_time_step, m_time_step);
...
#pragma omp critical
    ...
    if (coord.column > 0) {
      double tirage =
          pseudo_random(key * 13427 * 13427 * 13427 + m_time_step, m_time_step);
      double green_power = m_vegetation_map[key - 1];
      double correction = power * log_factor(green_power);
...
#pragma omp critical
    ...
    if (m_fire_front[key] == 255) { // On regarde si il commence à faiblir pour
