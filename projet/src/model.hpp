...
public:
  ...
  Model(double t_length, unsigned t_discretization,
...
private:
  std::size_t
  get_index_from_lexicographic_indices(LexicoIndices t_lexico_indices) const;
  ...
  std::array<double, 2> m_wind{
      0., 0.}; // Vitesse et direction du vent suivant les axes x et y en km/h
  double m_wind_speed; // Norme euclidienne de la vitesse du vent
  double m_max_wind;   //+ Vitesse à partir de laquelle le feu ne peut pas se
                       // propager dans le sens opposé à celui du vent.
