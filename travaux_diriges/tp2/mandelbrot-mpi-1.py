...
heightLoc = height // nbp
yMin = rank * heightLoc
yMax = (rank + 1) * heightLoc
print(rank, yMin, yMax)

scaleX = 3.0 / width
scaleY = 2.25 / height
convergenceLoc = np.empty((heightLoc, width), dtype=np.double)
# Calcul de l'ensemble de mandelbrot :
deb = time()
for y in range(yMin, yMax):
    for x in range(width):
        c = complex(-2.0 + scaleX * x, -1.125 + scaleY * y)
        convergenceLoc[y - yMin, x] = mandelbrot_set.convergence(c, smooth=True)

convergence = np.empty((height, width), dtype=np.double)
globCom.Gather(convergenceLoc, convergence, 0)
fin = time()
