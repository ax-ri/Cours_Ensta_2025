...
Matrix initTensorMatrices(const std::vector<double> &u,
...
bool verifProduct(const std::vector<double> &uA, std::vector<double> &vA,
                  const std::vector<double> &uB, std::vector<double> &vB,
                  const Matrix &C) {
  ...
  for (int irow = 0; irow < C.nbRows; irow++)
    for (int jcol = 0; jcol < C.nbCols; jcol++) {
      ...
      if (std::fabs(rightVal - C(irow, jcol)) >
          100 * std::fabs(C(irow, jcol) *
                          std::numeric_limits<double>::epsilon())) {
        std::cerr << "Erreur numérique : valeur attendue pour C( " << irow
                  << ", " << jcol << " ) -> " << rightVal
                  << " mais valeur trouvée : " << C(irow, jcol) << std::endl;
        return false;
      }
    }
  return true;
...
int main(int nargs, char *vargs[]) {
  ...
  if (isPassed) {
    ...
    std::cout << "Temps CPU produit matrice-matrice naif : "
              << elapsed_seconds.count() << " secondes\n";
    std::cout << "MFlops -> "
              << (2. * dim * dim * dim) / elapsed_seconds.count() / 1000000
              << std::endl;
  } else
