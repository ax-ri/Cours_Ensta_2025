...
x = 2. * np.random.random_sample((totalLocalSamples,)) - 1.
y = 2. * np.random.random_sample((totalLocalSamples,)) - 1.
...
approxLoc = np.array([4. * sum / nbSamples], dtype=np.double)
...
print(f"Pi vaut environ {approxGLob[0]}")
